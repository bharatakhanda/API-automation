param(
    [string]$OutputPath = ".\bin\api-automation.exe"
)
...
try {
    ...
    & wails3 generate bindings -b -names -d cmd/api-automation/frontend/src/bindings ./cmd/api-automation
    if ($LASTEXITCODE -ne 0) { throw "Wails binding generation failed" }

    Push-Location "cmd\api-automation\frontend"
    try {
    ...
    foreach ($asset in Get-ChildItem "cmd\api-automation\frontend\bundle" -File -Recurse) {
    ...
    & go build -tags production -trimpath -ldflags $ldflags -o $OutputPath .\cmd\api-automation
    if ($LASTEXITCODE -ne 0) { throw "Wails application build failed" }
    ...
    Write-Output ("Wails application build: PASS; version={0}; subsystem={1}; embedded-secret=yes; bytes={2}; timestamp={3:o}; sha256={4}" -f $expectedWails, $subsystem, $item.Length, $item.LastWriteTimeUtc, $hash)
} finally {
    $secret = $null
    $ldflags = $null
    Pop-Location
