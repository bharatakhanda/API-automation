...
const pages = {
  ...
  automation: ["Automation", "Inspect backend-owned plans, execute typed run modes, and cancel safely."],
...
async function testConnection() {
  ...
  try {
    ...
    notice(`${result.message}. Press Apply connection to unlock the workspace.`); appendLog(`Connection test passed for ${draft().ipAddress}.`);
  } catch (error) {
...
try {
  ...
  if (capabilityView) await refreshPresetList(); notice(`${state.version} is ready.`);
  setInterval(() => { if (currentPage === "overview" && state.connection?.hasActive) refreshOverview(); }, Math.max(500, metadata.overviewIntervalMs));
} catch (error) { notice(`Application backend failed to initialise: ${errorText(error)}`, true); }
