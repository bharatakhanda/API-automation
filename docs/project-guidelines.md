...
- Organize discovered features using the Antares/Capella/Vela Job Properties hierarchy: Job Info, Substrate/Media, Layout, Color, Image, Finishing, and VDP. Exclude Quick Access, keep Color and Image separate, vertically stack controls under matching nested headings/order, and retain Installable/Other only for advertised properties outside that hierarchy. Display every server-advertised value; do not impose an arbitrary UI-only value cap.
