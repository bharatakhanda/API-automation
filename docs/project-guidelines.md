...
- On Overview, poll documented `/live/api/v5/status` once per second while visible, log every result, and combine its health/extended status with application automation activity. An active run must display Busy even if Fiery returns stale `running/none`; otherwise use API-derived Idle/Busy/Unavailable. Keep the compact Server Details/header/reset layout and omit prior removed elements.
...
- Preserve every advertised `EFPageRange` value, including `Range1`, as an independent exact selection. Enable custom text only for range-capable `EFPageRange` metadata, normalize it, validate it against authoritative `OrigPageCount`, send it directly in `EFPageRange`, and require semantic readback from `EFPageRange`. Never send `DPP_PAGE_RANGE`, map custom text to `Range1`, accept the legacy field as substitute verification, or clamp invalid input silently.
