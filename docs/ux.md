...
- category tabs for Job Info, Substrate/Media, Layout, Color, Image, Finishing, and VDP, with Installable options and Other/Advanced retained only for discovered properties outside the report taxonomy; Quick Access is excluded,
- vertically stacked capability controls in Fiery Job Properties order under nested headings such as Job notes, Reporting, Die printing, Color input, Color settings, Edge enhancement, Advanced, Barcode, and Delivery option,
...
- local reusable presets for selections, numeric values, generation settings, run modes, and a safe server-preset ID; credentials and file paths are excluded,
- read-only discovery and selection of Fiery server presets; selected presets are applied after spooling and before explicit capability overrides, with no create/edit/delete controls,
...
- a separate Administration workspace for confirmed Fiery-process restart and server reboot plus a strongly guarded clear-all-jobs flow requiring a fresh inventory, exact typed phrase, server/count confirmation, immediate revalidation, and empty-inventory verification,
