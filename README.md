...
- Provides a compact Overview dashboard with matching blue header actions, Server Details without disk/memory rows, and one-second polling of the documented `/status` endpoint. Active application automation overrides a stale API `running/none` response to Busy; every poll is recorded diagnostically.
...
- Preserves every advertised `EFPageRange` value, including `Range1`, and enables validated custom ranges when the property advertises range-capable metadata. Custom expressions are sent directly as `EFPageRange`; `DPP_PAGE_RANGE` is never emitted.
